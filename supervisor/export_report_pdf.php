...
textarea[readonly] { background-color: #4a6fa5; color: white; }
</style>
</head>
<body>
...
<form method="POST" class="mb-2" id="form-<?= $row['report_id']; ?>">
    ...
    <div class="mb-2">
        ...
        <textarea name="supervisor_comments" 
                  ...
                  rows="3"
                  <?= $row['supervisor_comments'] ? 'readonly' : ''; ?>
                  style="<?= $row['supervisor_comments'] ? 'background-color:#4a6fa5; color:white;' : ''; ?>"
        ><?= htmlspecialchars($row['supervisor_comments']); ?></textarea>
    </div>
...
document.querySelectorAll('.comment-btn').forEach(button => {
    button.addEventListener('click', function() {
        ...
        if (textarea.hasAttribute('readonly')) {
            ...
            document.getElementById('form-' + reportId).submit();
        }
    });
});
</script>
